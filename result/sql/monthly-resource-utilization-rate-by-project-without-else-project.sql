-- [메인: 4~9월, 주말 제외, 제외항목 제거 후 월별 합=100% / 기간 합=100%]
WITH
params AS (
...
/* 0-1) 최종 필터 + 대분류 재분류 적용
   ...
   - 기타업무: 지정 4개 프로젝트만 포함 + cat을 '제품개발(엔진)'으로 재분류, 나머지 기타업무는 제외
   - 교육/연차(반차): 제외
*/
raw AS (
  SELECT
...
/* 12) (재정의) 월별 분모 = '제외항목 제외 후 포함된 프로젝트들의 월별 총합'  → 월별 합계가 정확히 100% */
...
/* 12-1) 프로젝트별 월별 퍼센트(잔여 프로젝트 집합 기준) */
cat_proj_month_pct AS (
  SELECT
    m.ymonth,
    m.cat, m.proj,
    ROUND(m.proj_hours * 100.0 / NULLIF(t.month_included_hours, 0), 2) AS pct_month
  FROM cat_proj_month m
  JOIN month_included_total t ON t.ymonth = m.ymonth
),
...
/* 14-1) (재정의) 기간 총합의 분모 = '잔여 프로젝트들의 4~9월 총합'  → 기간 합계가 정확히 100% */
period_included_total AS (
  SELECT SUM(proj_total_hours) AS period_included_hours
  FROM cat_proj_period
)
...
SELECT
  ...
  /* 기간 누적(잔여 기준) → 합계 100% */
  ROUND(cpp.proj_total_hours * 100.0 / NULLIF(pit.period_included_hours, 0), 2)
                                            AS "총합_%",

  /* 표준용량 대비(참고지표, 합계 100% 아님) */
  ...
  ROUND( (COALESCE(p."2025-04_%",0)+COALESCE(p."2025-05_%",0)+COALESCE(p."2025-06_%",0)
        + COALESCE(p."2025-07_%",0)+COALESCE(p."2025-08_%",0)+COALESCE(p."2025-09_%",0))
          / NULLIF( (CASE WHEN p."2025-04_%" IS NOT NULL THEN 1 ELSE 0 END
                   + CASE WHEN p."2025-05_%" IS NOT NULL THEN 1 ELSE 0 END
                   + CASE WHEN p."2025-06_%" IS NOT NULL THEN 1 ELSE 0 END
                   + CASE WHEN p."2025-07_%" IS NOT NULL THEN 1 ELSE 0 END
                   + CASE WHEN p."2025-08_%" IS NOT NULL THEN 1 ELSE 0 END
                   + CASE WHEN p."2025-09_%" IS NOT NULL THEN 1 ELSE 0 END), 0), 2) AS "평균_%"
...
ORDER BY "총합_%" DESC, "총_프로젝트시간" DESC;
